...
この MOD は、C# やゲーム開発の学習を目的として個人制作したものです。
...
- **独自リソースの永続化** — MP の現在値・最大値を JSON で保存。戦闘中断再開やセーブスキャムに対する値の巻き戻し対策を実装
- **Harmony パッチによる UI 拡張** — HP バー下に MP バーを動的に描画。公式の Star（セカンダリコスト）UI に乗ることで、カードにバッジノードを足さずに消費 MP を表示
- **1 枚絵 PNG からの立ち絵生成** — `CustomVisualPath` 経由で通常の Node2D シーンを `NCreatureVisuals` に自動変換。AnimationTree の StateMachine で idle 自動復帰を実現
- **少ないフレームからのアニメーション制作** — 限られた枚数の差分画像をベースに、Sprite の位置・スケール・再生速度のキー操作を組み合わせて attack などの動きを構成
- **商人・焚き火立ち絵** — `NMerchantCharacter` / `NRestSiteCharacter` の生成仕様に合わせた Node2D シーン構成
...
BaseLib・STS2 の MOD 資料は Web 上に少なく、十分な情報が得られません。
そのため、**ゲーム本体・BaseLib をデコンパイルして一次情報を直接読む** ことで開発を進めました。

- **動く例 / 動かない例を並べて差分を見る** — バグの原因が不明なとき、正常動作するカードと異常なカードを並べて共通点・相違点から原因構造を浮かび上がらせる手法を多用

ドキュメントの整備されていない領域で一次情報を自力で掘り当てる——この方法を主に開発しました。
...
アートは AI を使って制作しています。しかし、すべてのアートに手間をかけて制作しました。

どのカードも、まず頭の中で構図を思い描くところから始まります。それをプロンプトとして
言葉にし、生成し――そして思った通りのものはなかなか出てこない（特に手や生物、色やタッチのテイスト）――
自分で納得できるまで描き直し、加工しています。
すべてのアートに時間を使い、そして、こだわっています。

AI はあくまで道具です。構図も、直しも、思い入れも、私のものです。
AI 生成のアートが好きでないのは存じていますが、どうか私の作品を嫌いにならないでほしいです。
