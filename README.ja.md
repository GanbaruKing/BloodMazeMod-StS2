...
- 追加キャラクター: **レヴナント**
- 独自のカード **87 枚**（アタック / スキル / パワー、各レアリティ、協力用カード）
- 新しいバフとデバフを追加
- 協力モード対応
...
- **カード 87 枚** — MP を消費するというコストの代わりに、汎用的で強力な効果を持つカードが多数
...
4. **ゲームを起動する**
   起動すると MOD を検知したメッセージが表示されるので、承認すると レヴナント が使用可能になります。(設定からMODの有効をONにする。)
...
このプロジェクトのアートはAIを補助的に使って制作しています。
